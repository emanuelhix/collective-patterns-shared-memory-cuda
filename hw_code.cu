  ...
  // #pragma omp parallel
  {
    ...
    */
    if (tid == root_tid)
    {
      ...
      {
        output_sequential[root_tid] += shared_memory[src_tid]; // FIXME!
      }
    }
    /*
      END_STUDENT_TODO
  ...
  // #pragma omp parallel
  {
    ...
    */
    if (tid == root_tid)
    {
      for (int src_tid = 0; src_tid < num_threads; src_tid++)
      {
        output_sequential[src_tid + root_tid * num_threads] = shared_memory[src_tid]; // FIXME!
      }
    }
    /*
      END_STUDENT_TODO
                                   ...
                                   float *output_sequential)
{
  ...
  // #pragma omp parallel
  {
    ...
    */
    ...
    for (int src_tid = 0; src_tid < num_threads; src_tid++)
    {
      output_sequential[tid] += shared_memory[src_tid]; // FIXME!
    }
                                   ...
                                   float *output_sequential)
{
  ...
  //  #pragma omp parallel
  {
    ...
    */
    for (int src_tid = 0; src_tid < num_threads; src_tid++)
    {
      output_sequential[src_tid + tid * num_threads] = src_sequential[src_tid]; // FIXME!
    }
                                   ...
                                   float *output_sequential)
{
  ...
  // #pragma omp parallel
  {
    ...
    */
    for (int i = 0; i < num_threads; ++i)
    {
      int out_idx = tid * num_threads + i;
      int shared_idx = i * num_threads + tid;
      output_sequential[out_idx] = shared_memory[shared_idx];
    }
    /*
      END_STUDENT_TODO
    */
